...
export function PlanApprovalCard({
  plan, onRespond,
}: { plan: LessonPlan; onRespond: (r: ApprovePlanResponse) => void }) {
  return (
    <div className="rounded-md border border-border bg-surface p-6 shadow-card">
      ...
      <ul className="mt-4 space-y-3">
        {plan.objectives.map((o, i) => (
          <li key={i} className="flex items-start gap-3">
            <span className="mt-0.5 inline-flex h-6 w-6 items-center justify-center rounded-sm text-xs font-semibold text-white"
                  style={{ backgroundColor: ACCENTS[i % ACCENTS.length] }}>{i + 1}</span>
            <div>
      ...
      <div className="mt-6 flex gap-3">
        <button className="rounded-md bg-primary px-4 py-2 font-medium text-white"
                onClick={() => onRespond({ approved: true })}>Approve & start</button>
        <button className="rounded-md border border-border px-4 py-2 font-medium"
                onClick={() => onRespond({ approved: false })}>Ask for changes</button>
      </div>
    </div>
  );
}
